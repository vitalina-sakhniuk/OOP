...
class Manager :public Employee, public Pryimach{
	public:
		...
		template <typename GetsalaryTYPE> GetsalaryTYPE getsalary(GetsalaryTYPE a){
			return a;
		};
};
...
public:
	...
	ReloadedOperators operator++(int);
	ReloadedOperators operator++(){ x++; return *this; };
};
...
class Passwords{
private:
	T1 passwordFORadmin, passwordFORmanager;
	T2 login;
public:	
	int passADM(T1 passwordFORadmin){ return passwordFORadmin; };
	int passMAN(T1 passwordFORmanager){ return passwordFORmanager; };
	void enterpassword(T1 passwordFORadm, T1 passwordFORman){
